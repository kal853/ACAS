...
*>
 copy "selboitm.cob".
     select  boitm-File-Seq  assign        "bostkitm.seq"
...
*>----------------------
 77  prog-name           pic x(20)    value "boitmRES (3.3.01)".
...
*>
 01  Error-Messages.
     ...
     03  SY006          pic x(33) value "SYS06 Write error on .dat file = " .
...
*>
     ...
     if       FS-Reply not = zeros
              move     spaces to SO-Print
              string   "Failed on open - bostkitm file = "
                       FS-Reply
                           into SO-Print
              perform  Call-Sysout
              move     "CLOSE " to SO-Print
...
*>
 ...
 aa010-Read-Recs.
     perform  forever
              read     boitm-File-Seq next record at end
                       exit perform
              end-read
              if       FS-Reply not = zeros
                       string   SY003
