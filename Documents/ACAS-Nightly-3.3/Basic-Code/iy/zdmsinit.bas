...
rem
rem             display interface initialization
rem
rem----------------------------------------------------------------------------
rem
...
rem     ----------------------------------------------------
rem     uses fn.crt.cmdstr$ on string reads
rem     added code to read control character translation tables
rem----------------------------------------------------------------------------
rem
rem     fn.crt.cmdstr$          read and format crt device command strings
rem
rem----------------------------------------------------------------------------
...
rem
   def fn.crt.initialize% (crt.def.file$,crt.disk$)
        fn.crt.initialize% = 0
        crt.dummy$ = fn.crt.dim.screen.buffer$ (crt.max.buffer.count%)
crt.format$="###,###,###,###,###"
dim crt.digits%(15)
crt.digits%(01)=1: crt.digits%(02)=2: crt.digits%(03)=3
crt.digits%(04)=5: crt.digits%(05)=6: crt.digits%(06)=7
crt.digits%(07)=9: crt.digits%(08)=10: crt.digits%(09)=11
crt.digits%(10)=13: crt.digits%(11)=14: crt.digits%(12)=15
crt.digits%(13)=17: crt.digits%(14)=18: crt.digits%(15)=19
        crt.order.row.col% = -1
        crt.order.col.row% = 0
        crt.sca.decimal% = 1
        crt.sca.hex% = 0
...
crt.init.console.data$=fn.crt.cmdstr$
        ...
        if crt.init.console.data$ <> "" \
          then  console :       \
                print using crt.using$;crt.init.console.data$
